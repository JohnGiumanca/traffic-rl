...
if __name__ == '__main__':
    ...
    env = SumoEnv(net_path=NET_PATH, rou_path=new_routefile, max_steps=MAX_STEPS, sumo_gui=gui, 
    ...
    dqn = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, nb_steps_warmup=2000,
                   enable_dueling_network=True, dueling_type='avg', target_model_update=0.001, policy=policy)
    dqn.compile(Adam(lr=0.001), metrics=['mae'])

    if args.train:  
        # dqn.load_weights('dqn_traffic_weights.h5f')
        history = dqn.fit(env, nb_steps=50_000, visualize=False, verbose=2)
